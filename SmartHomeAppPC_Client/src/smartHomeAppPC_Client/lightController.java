...
public class lightController {
	...
	public static DigitalOutput lightExists(Integer serial, Integer channel) { // check if a light (of a specific serial and channel) already exists
		for (int i = 0; i < lightList.size(); i++) { // loop through list of added lights
            try {
				if(lightList.get(i).getDeviceSerialNumber() == serial && lightList.get(i).getChannel() == channel) { // check if serial and channel are the same
	...
	public static boolean checkLight(String serialChannel) { // check the state of a specific light
		...
		DigitalOutput light = lightExists(serialNumber, channel); // set light to returned light after checking it exists
